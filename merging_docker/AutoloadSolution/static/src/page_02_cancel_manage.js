...
class CancelManage extends Component {
    ...
    applySorting() {
        ...
        sorted.sort((a, b) => {
            ...
            // 날짜 정렬
            if (this.state.sortField === 'date' || this.state.sortField === 'created_at') {
    ...
    async handleIssueInvoice(item) {
        try {
            ...
            const vehicleResponse = await fetch('/autoloadsolution/vehicleinfo/list', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
            ...
            if (!vehicleResponse.ok) {
                throw new Error(`Vehicle info HTTP error! status: ${vehicleResponse.status}`);
            }

            const vehicleResult = await vehicleResponse.json();
            
            if (vehicleResult.error) {
                throw new Error(vehicleResult.error.data.message || vehicleResult.error.message);
            }

            // 2. vehicle_id와 일치하는 데이터 찾기
            const vehicleInfo = vehicleResult.result.find(v => v.vehicle_id === item.vehicle_id);
            ...
            if (!pdfResponse.ok) {
                throw new Error(`PDF generation HTTP error! status: ${pdfResponse.status}`);
            }
